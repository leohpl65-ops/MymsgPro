...
export function UserSettingsModal({
  ...
  const handleSave = () => {
    ...
    if (lowerName === "owner") {
      import("@/hooks/use-toast").then(({ toast }) => {
        toast({
          description: "No te puedes poner ese nombre",
          variant: "destructive",
          duration: 3000,
        });
      });
      ...
      >
        ...
        <div className="flex flex-col items-center gap-4 py-4">
          ...
          <div className="w-full pt-2">
            ...
            <br />
            <Label className="text-xs text-muted-foreground">
              Usuario: {currentUser?.originalName || currentUser?.name}
            </Label>
          </div>
                ...
                {linkMode === "manual" ? (
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (googleEmail.trim()) {
                          ...
                          import("@/hooks/use-toast").then(({ toast }) => {
                            toast({
                              description: "Cuenta vinculada correctamente",
                              duration: 3000,
                            });
                          });
                        }
                  ...
                  <div className="flex flex-col items-center gap-4 py-4">
                    ...
                    <p className="text-center text-sm text-muted-foreground">
                      Haz clic abajo para vincular tu cuenta de Google actual.
                    </p>
                    <Button
                      className="w-full"
                      onClick={async () => {
                        // Using Firebase Auth for real Google Sign-In
                        try {
                          const {
                            getAuth,
                            signInWithPopup,
                          ...
                          // NOTE: If testing locally or on Vercel, ensure the domain is added
                          // to the Firebase Console -> Authentication -> Settings -> Authorized domains
                          const result = await signInWithPopup(auth, provider);
                          const user = result.user;

                          if (user && user.email) {
                            updateUser({ googleLinked: user.email });
                            setShowGoogleDialog(false);
                            import("@/hooks/use-toast").then(({ toast }) => {
                              toast({
                                description: `Cuenta vinculada correctamente con ${user.email}`,
                                duration: 3000,
                              });
                            });
                          }
                          ...
                          if (error.code === "auth/unauthorized-domain") {
                            errMsg =
                              "Este dominio no está autorizado en Firebase. Contacta al administrador.";
                          } else if (
              ...
              <div className="space-y-4 py-4">
                ...
                {!currentUser?.googleLinked && pwdMode === "email" && (
                  <p className="text-xs text-red-500">
                    Debes vincular una cuenta de Google primero.
                  </p>
                )}
              ...
              <div className="space-y-4 py-4 text-sm">
                <div>
                  <h3 className="font-bold text-base mb-1">
                    1. Cómo añadir amigos
                  </h3>
                  <p className="text-muted-foreground">
                    Primero mira en tus configuraciones tu ID. Luego tu amigo en
                    la sección de agregar amigos debe poner esa ID, también
                    pueden hacerlo al revés.
                  </p>
                </div>
                <div>
                  <h3 className="font-bold text-base mb-1">
                  ...
                  <p className="text-muted-foreground">
                    Ve al botón en la pantalla de inicio que tiene 👥+ y entra a
                    "Crear grupo", ponle un nombre y listo.
                  </p>
                </div>
                <div>
                  <h3 className="font-bold text-base mb-1">
                    3. Cómo unirse a un grupo
                  </h3>
                  <p className="text-muted-foreground">
                    Similar a cómo agregar a un amigo, ve a las configuraciones
                    del grupo, allí hay una ID. Tu amigo debe entrar a 👥+ y
                    poner ese ID y listo.
                  </p>
                </div>
                <div>
                  <h3 className="font-bold text-base mb-1">
                    4. Cómo hacer copias de seguridad
                  </h3>
                  <p className="text-muted-foreground">
                    Ve a "Mi Perfil" desde el menú de inicio y presiona "Hacer
                    copia". Esto guardará todos tus mensajes y contactos para
                    que no los pierdas.
                  </p>
                </div>
            ...
            <DialogContent className="sm:max-w-xs">
              ...
              <div className="py-4">
                <p className="text-sm text-muted-foreground mb-4">
                  ¿Estás seguro de que quieres cerrar sesión de la cuenta{" "}
                  <strong>
                    {currentUser?.originalName || currentUser?.name}
                  </strong>{" "}
                  (ID: {currentUser?.id})?
                </p>
            ...
            <DialogContent className="sm:max-w-xs max-h-[85vh] overflow-y-auto">
              ...
              <div className="w-full space-y-2 border-t pt-4">
                ...
                <p className="text-xs text-muted-foreground">
                  Si añades tu canal, otros verán un icono junto a tu nombre.
                </p>
                <Button
                  onClick={() => setShowExtraOptions(false)}
                  className="w-full mt-2"
                ...
                {notificationsEnabled ? (
                  ...
                  <div className="bg-slate-50 p-3 rounded-md border border-slate-200 space-y-2">
                    <p className="text-sm text-slate-600 text-center">
                      Las notificaciones no están activadas
                    </p>
                    <Button
          ...
          <div className="w-full pt-4 border-t text-center">
            <p className="text-xs text-blue-600 font-semibold">
              Derechos a Leonardo Humaza Poiqui
            </p>
          </div>
        </div>
      </DialogContent>
                ...
                onClick={async () => {
                  if (joinGroupId.trim()) {
                    try {
                      ...
                      const fullGroupId = joinGroupId.startsWith("group-")
                        ? joinGroupId
                        : `group-${joinGroupId}`;

                      const groupSnap = await get(
                        ref(db, `groups/${fullGroupId}`),
                      );
                      if (groupSnap.exists()) {
  ...
  const handleAddContact = async () => {
    ...
    try {
      ...
      if (userSnap.exists()) {
        ...
        if (
          cleanFriendId.toLowerCase() === "ia" ||
          cleanFriendId.toLowerCase() === "mymsgai"
        ) {
          targetId = "mymsgai";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        aria-describedby="add-friend-dialog"
      >
        <DialogHeader>
              ...
              <div className="text-xs mt-1">
                {foundUser.inApp ? (
                  <span className="text-green-600 font-medium">
                    ✅ {foundUser.name} (está en mymsg pro)
                  </span>
                ) : (
...
export function CallHistoryModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="sm:max-w-xs"
        aria-describedby="call-history-dialog"
      >
        <DialogHeader>
            ...
            <div className="space-y-3">
              {callHistory.map((call, idx) => (
                ...
                >
                  ...
                  <div
                    className="flex-1 min-w-0 cursor-pointer hover:bg-slate-100 rounded p-1 transition-colors"
                    onClick={() => {
                      ...
                      if (call.peerId) {
                        ...
                        setTimeout(() => {
                          window.dispatchEvent(
                            new CustomEvent("mymsg-start-call", {
                              detail: { peerId: call.peerId },
                            }),
                          );
                        }, 500);
                        onOpenChange(false);
...
export function ReportsModal({
  ...
  const isModerator =
    useStore().currentUser?.id === "Owner333" ||
    useStore().currentUser?.id === "12345670";
  ...
  const handleBan = async (
    ...
    toast({
      description: `El usuario ${targetName} ha sido baneado exitosamente del sistema.`,
      duration: 3000,
    });
          ...
          {reports.length === 0 ? (
            ...
            reports.map((r) => (
              <motion.div
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="border rounded-lg bg-red-50 dark:bg-red-950 overflow-hidden"
              >
                <button
                  onClick={() =>
                  ...
                  >
                    <div className="max-h-[250px] overflow-y-auto mb-4 space-y-2">
                      <p className="text-xs font-bold text-muted-foreground mb-3">
                        Últimos {r.targetMessages.length} mensajes:
                      </p>
                      {r.targetMessages.map((msg) => (
                        <div
                          key={msg.id}
                          className="bg-background p-2 rounded text-[11px] border-l-2 border-yellow-500"
                        >
